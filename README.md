...
```
projeto/
├── legacy.py                        ← Código legado original (não modificado no Sprint 0)
├── src/
│   ├── main.py
│   ├── models/
│   │   ├── order.py
...
├── tests/
│   ├── golden_master/
│   │   └── test_legacy_behavior.py  ← 31 testes, 94% de cobertura
│   ├── unit/
│   └── integration/
├── docs/
│   ├── analise.md
...
### 🔄 Sprint 1 — SRP, ISP e separação em camadas (até 20/05/2026)
- Organização em camadas: models, repositories, services, interfaces
- Padrão Repository isolando acesso ao SQLite
- Interfaces abstratas (ABC) por camada

### ⏳ Sprint 2 — OCP, DIP, LSP e padrões GoF (até 26/05/2026)
- Strategy para descontos e pagamentos
- Observer para notificações
- Factory Method para criação de pedidos
- Injeção de dependência explícita
- Três extensões obrigatórias
...
| Padrão | Aplicação |
|---|---|
| **Strategy** | Algoritmos de desconto e métodos de pagamento intercambiáveis |
| **Repository** | Isolamento do acesso a SQLite |
| **Observer** | Notificações desacopladas da regra de negócio |
| **Factory Method** | Criação de pedidos diferenciados por tipo de cliente |
...
- [ ] **Pagamento em criptomoeda** — taxa de 2% sobre o valor do pedido
- [ ] **Canal WhatsApp** — notificações para todos os tipos de cliente
- [ ] **Desconto por volume** — 3+ unidades do mesmo item recebem 15% adicional
